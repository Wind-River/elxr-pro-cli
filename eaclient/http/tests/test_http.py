...
class TestIsHttpsUrl:
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("https://asdf/", True),
            ("http://host:port", False),
            ("https://asdf:1234", True),
        ),
    )
    def test_is_https_url(self, url, is_https):
        ret = http.is_https_url(url)
        assert is_https is ret
...
class TestValidateProxy:
    ...
    def test_fails_on_invalid_url(self, m_open, proxy):
        ...
        """
        with pytest.raises(exceptions.ELxrProError) as e:
    ...
    @mock.patch("urllib.request.OpenerDirector.open")
    @mock.patch("eaclient.http._readurl_pycurl_https_in_https")
...
class TestReadurl:
    ...
    @mock.patch("eaclient.http._readurl_urllib")
    def test_readurl(
        self,
        m_readurl_urllib,
        url,
