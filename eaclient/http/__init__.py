...
EA_NO_PROXY_URLS = ("169.254.169.254", "metadata", "[fd00:ec2::254]")
PROXY_VALIDATION_APT_HTTP_URL = "http://mirror.elxr.dev/"
PROXY_VALIDATION_APT_HTTPS_URL = "https://mirror.elxr.dev/"
...
def configure_web_proxy(
    ...
    no_proxy = ",".join(sorted(EA_NO_PROXY_URLS))
    for env_var in ("no_proxy", "NO_PROXY"):
        proxy_value = os.environ.get(env_var)
        if proxy_value:
            # Honor no proxy values and extend EA-specific where absent
            no_proxy = ",".join(
                sorted(
                    set(proxy_value.split(",")).union(set(EA_NO_PROXY_URLS))
                )
            )
